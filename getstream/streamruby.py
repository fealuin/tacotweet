...
def ifTaco(text):
    palabrasTaco=['trafico','congestion','transito','semaforo']
    i=0
    tweet=text.lower().split(' ')
    for palabra in palabrasTaco:
        if palabra in tweet:
            i=i+1
    if 'taco' in tweet and 'en' in tweet:
        i=i+1
    return i

def ifAccidente(text):
    palabras=['accidente','accidentes','panne','choque','volcamiento','colision']
    i=0
    tweet=text.lower().split(' ')
...
#            print(status.coordinates[u'coordinates'])
            ...
            data_tweet=(status.text,status.created_at,status.retweet_count,status.user.id_str,status.source,str(lat)+"|"+str(lon),status.user.geo_enabled,status.id_str)
